...
class DDPG(object):
    def __init__(self, args):
        ...
        if self.args.replay_buffer_choice == 1: self.replay_buffer = ReplayBuffer(self.args.buffer_size, random.randint(1,10000))
    ...
    def update_actor(self, episode):
        ...
        action_T = self.ac.actor_forward(states)
        policy_loss = -self.ac.critic_forward(states, action_T)
    ...
    def update_critic(self, episode):
        # Sample batch
        states, next_states, actions, rewards, data = self.sample_memory(episode)
        actions = to_tensor(np.array(actions)).t()
        if len(states) == 0: return


        for epoch in range(self.args.critic_epoch):

            vals = self.ac.critic_forward(states, actions);
            next_action = self.ac.actor_forward(next_states)


            next_states.volatile = True
            new_vals = self.ac.critic_forward(next_states, next_action)
...
class Actor_Critic(nn.Module):
    def __init__(self, num_input, num_actions, args):
        ...
        #Critic
        self.w_critic1 = Parameter(torch.rand(1, args.num_hnodes+num_actions), requires_grad=1)
